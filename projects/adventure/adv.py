...
from util import Stack, Queue # not using
...
# populate new graph with keys/nodes from roomGraph
for key in roomGraph:
    if key not in graph:
        graph[key] = {}

# populate new graph with edges from roomGraph 
# (the directions for each key in roomGraph)
for key in roomGraph:
    for value in roomGraph[key][1]:
        # print("Val ", value)
        # adds room id as values to dir
        # graph[key][value] = roomGraph[key][1][value]

        # adds question mark as values to dir
        graph[key][value] = '?'
...
while len(explored) != len(graph): 
    ...
    if len(moveOptions) == 0 and len(goBack) == 0:
        break
    ...
    backwardsMove = oppositeDir(nextMove)
    goBack.append(backwardsMove)

    # Get the id of the room in next move direction

    if nextMove == "n":
        nextRoomId = player.currentRoom.n_to.id 
    if nextMove == "s":
        nextRoomId = player.currentRoom.s_to.id 
    if nextMove == "e":
        nextRoomId = player.currentRoom.e_to.id 
    if nextMove == "w":
...
print("Traversed Path ", traversalPath)

# Handle Dead Ends While Loop:
